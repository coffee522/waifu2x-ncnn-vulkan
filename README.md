...
ncnn implementation of waifu2x with pipelined lossy WebP output. It runs on Intel / AMD / Nvidia / Apple-Silicon with Vulkan API.

waifu2x-ncnn-vulkan uses [ncnn project](https://github.com/Tencent/ncnn) as the universal neural network inference framework.

GPU upscaling, image loading, and CPU WebP encoding run as bounded concurrent stages. A job succeeds only after every requested output has been encoded and committed.
...
```shell
waifu2x-ncnn-vulkan.exe -i input.jpg -o output.webp -n 2 -s 2
```
...
```shell
waifu2x-ncnn-vulkan.exe -l chapter-001.tsv -n 2 -s 2 -j 1:2:12
```

`1:2:12` matches the tested Ryzen 5 5600 setup (one loader, two GPU jobs, twelve independent WebP encoders). Tune it for the target machine and image dimensions; the built-in default remains `1:2:2`.
...
Usage: waifu2x-ncnn-vulkan -i infile -o outfile [options]...
  ...
  -o output-path       output WebP image path or directory
  -l list-path         UTF-8 TSV with input-path<TAB>output-webp-path per line
...
- `input-path` accepts JPG, PNG, or WebP. Output is always lossy WebP.
...
- Output parent directories must already exist. Final `.webp` files and their `.tmp` paths must not exist before the job starts.
...
- `load:proc:save` = independent worker counts for image decoding, waifu2x processing, and WebP encoding. Each WebP encoder is single-threaded internally, so parallelism comes from the `save` worker count. More workers increase peak memory use.
